...
(function() {
  ...
  MyRawStream.prototype.write = function(rec) {
    ...
    } else if (config.report.type == 'http') {
      process.nextTick(function() {
        _toHTTP(rec);
      });
  ...
  function _toKafka(rec) {
    ...
    var producer = new Producer({
      ...
      if (success) {
        ...
        var jsonStr = JSON.stringify(rec)
        var base64Str = new Buffer(jsonStr).toString('base64');
        ...
        producer.produce(produceRequest, function(response) {
          ...
          if (response
            && response.topicsAndPartitions
            && (response.topicsAndPartitions instanceof Array)
            && response.topicsAndPartitions[0]
            && response.topicsAndPartitions[0].partitions
            && (response.topicsAndPartitions[0].partitions instanceof Array)
            && response.topicsAndPartitions[0].partitions[0].errorCode === 0) {
